...
class TestMistralTokenizer:
    ...
    def test_tokenize_messages_special_tokens(
    ...
    def test_inference_drops_eos(self, messages, expected_tokens):
        tokenizer = self.tokenizer(template=False)
        sample = {"messages": messages}
        result = tokenizer(sample, inference=True)

        assert result["tokens"] == expected_tokens[:-1]
        assert result["mask"] == [True] * 75 + [False] * 124
